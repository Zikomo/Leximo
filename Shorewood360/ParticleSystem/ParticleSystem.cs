...
namespace Particle3DSample
{
    ...
    public abstract class ParticleSystem : DrawableGameComponent
    {
        ...
        void LoadParticleEffect()
        {
            ...
            texture = content.Load<Texture2D>(settings.TextureName);
            if (settings.TextureName1337 != null)
            {
                texture1337 = content.Load<Texture2D>(settings.TextureName1337);
            }

            parameters["Texture"].SetValue(texture);
            parameters["sparkle"].SetValue(settings.IsSparkling);
        ...
        public void ChangeTexture(bool to1337)
        {
            EffectParameterCollection parameters = particleEffect.Parameters;
            if (to1337)
            {
                if (texture1337 != null)
                {
                    parameters["Texture"].SetValue(texture1337);
                    if (settings.IsSparkling)
                    {
                        parameters["MinColor"].SetValue(Color.Red.ToVector4());
                        parameters["sparkle"].SetValue(false);
                    }
                }
            }
            else
            {
                if (texture != null)
                {
                    ...
                    {
                        parameters["MinColor"].SetValue(settings.MinColor.ToVector4());
                        parameters["sparkle"].SetValue(true);
                    }
                }
