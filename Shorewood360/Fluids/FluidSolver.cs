    ...
    public class FluidSolver
    {
        ...
        public void buoyancy(float[] Fbuoy)
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                for (int j = 1; j <= n; j++)
                {
                    Fbuoy[(i) + (n + 2) * (j)] = a * d[(i) + (n + 2) * (j)] + -b * (d[(i) + (n + 2) * (j)] - Tamb);
                }
            }
        }
        ...
        public float curlFunction(int i, int j)
        {
            float du_dy = (u[(i) + (n + 2) * (j + 1)] - u[(i) + (n + 2) * (j - 1)]) * 0.5f;
            float dv_dx = (v[(i + 1) + (n + 2) * (j)] - v[(i - 1) + (n + 2) * (j)]) * 0.5f;
        ...
        public void vorticityConfinement(float[] Fvc_x, float[] Fvc_y)
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                for (int j = 1; j <= n; j++)
                {
                    curl[(i) + (n + 2) * (j)] = Math.Abs(curlFunction(i, j));
                }
            }
            ...
            for (int i = 2; i < n; i++)
            {
                for (int j = 2; j < n; j++)
                {
                    ...
                    dw_dx = (curl[(i + 1) + (n + 2) * (j)] - curl[(i - 1) + (n + 2) * (j)]) * 0.5f;
                    //dw_dy = (curl[I(i, j + 1)] - curl[I(i, j - 1)]) * 0.5f;
                    dw_dy = (curl[(i) + (n + 2) * (j + 1)] - curl[(i) + (n + 2) * (j - 1)]) * 0.5f;
                    ...
                    // N x w
                    Fvc_x[(i) + (n + 2) * (j)] = dw_dy * -v;
                    Fvc_y[(i) + (n + 2) * (j)] = dw_dx * v;
                }
            }
        }
        ...
        void project(float[] x, float[] y, float[] p, float[] div)
        {
            for (int i = 1; i <= n; i++)
            {
                for (int j = 1; j <= n; j++)
                {
                    ...
                    //              + y[I(i, j + 1)] - y[I(i, j - 1)])
                    //              * -0.5f / n;
                    div[(i) + (n + 2) * (j)] = (x[(i + 1) + (n + 2) * (j)] - x[(i-1) + (n + 2) * (j)]
                        + y[(i) + (n + 2) * (j + 1)] - y[(i) + (n + 2) * (j-1)])
                        * -0.5f / n;
            ...
            for (int i = 1; i <= n; i++)
            {
                for (int j = 1; j <= n; j++)
                {
                    ...
                    x[(i) + (n + 2) * (j)] -= 0.5f * n * (p[(i + 1) + (n + 2) * (j)] - p[(i-1) + (n + 2) * (j)]);
                    //y[I(i, j)] -= 0.5f * n * (p[I(i, j + 1)] - p[I(i, j - 1)]);
                    y[(i) + (n + 2) * (j)] -= 0.5f * n * (p[(i) + (n + 2) * (j + 1)] - p[(i) + (n + 2) * (j - 1)]);
                }
            }
            ...
            for (int k = 0; k < 20; k++)
            {
                for (int i = 1; i <= n; i++)
                {
                    for (int j = 1; j <= n; j++)
                    {
                        ...
                        //                + x[I(i, j - 1)] + x[I(i, j + 1)])
                        //                + x0[I(i, j)]) / c;
                        x[i + (n + 2) * j] = (a * (x[(i - 1) + (n + 2) * j] + x[(i + 1) + (n + 2) * (j)]
                                        + x[(i) + (n + 2) * (j - 1)] + x[(i) + (n + 2) * (j + 1)])
                                        + x0[(i) + (n + 2) * (j)]) / c;
                    }
                }
                setBoundry(b, x);
